...
class SoftwareDetailViewController: UIViewController {
    ...
    private var softwareDetail: Search?
    private let context = appDelegate.persistentContainer.viewContext
    
    init(softwareDetail: Search) {
        self.softwareDetail = softwareDetail
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = softwareDetail?.artworkUrl600 ?? softwareDetail?.artworkUrl100
    ...
    private func setupUI() {
        searchName.text = "Name: \(softwareDetail?.trackName ?? "")"
        searchPrice.text = "Price: \(softwareDetail?.trackPrice ?? 0.0)" 
        searchDescription.text = softwareDetail?.longDescription ?? softwareDetail?.shortDescription ?? softwareDetail?.resultDescription ?? ""
        getImage(value: softwareDetail?.artworkUrl600 ?? softwareDetail?.artworkUrl100 ?? "")
    }

...
class MusicDetailViewController: UIViewController {
    ...
    private var musicDetail: Search?
    private let context = appDelegate.persistentContainer.viewContext
    
    init(musicDetail: Search) {
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = musicDetail?.artworkUrl600 ?? musicDetail?.artworkUrl100
    ...
    private func setupUI() {
        searchName.text = "Name: \(musicDetail?.trackName ?? "")"
        searchPrice.text = "Price: \(musicDetail?.trackPrice ?? 0.0)"
        searchDescription.text = musicDetail?.longDescription ?? musicDetail?.shortDescription ?? musicDetail?.resultDescription ?? ""
        getImage(value: musicDetail?.artworkUrl600 ?? musicDetail?.artworkUrl100 ?? "")
    }

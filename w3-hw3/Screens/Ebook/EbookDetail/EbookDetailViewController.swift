...
class EbookDetailViewController: UIViewController {
    ...
    private var ebookDetail: Search?
    private let context = appDelegate.persistentContainer.viewContext
    
    init(ebookDetail: Search) {
        self.ebookDetail = ebookDetail
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = ebookDetail?.artworkUrl600 ?? ebookDetail?.artworkUrl100
        favorite.name = ebookDetail?.trackName
        if let price = Double(ebookDetail?.formattedPrice ?? "") {
            favorite.price = price
        }
        // favorite.price = ebookDetail?.formattedPrice ?? ""
    ...
    private func setupUI() {
        searchName.text = "Name: \(ebookDetail?.trackName ?? "")"
        searchPrice.text = "Price: \(ebookDetail?.formattedPrice ?? "")"
        searchDescription.text = ebookDetail?.longDescription ?? ebookDetail?.shortDescription ?? ebookDetail?.resultDescription ?? ""
        getImage(value: ebookDetail?.artworkUrl600 ??  ebookDetail?.artworkUrl100 ?? "")
    }

...
class MovieDetailViewController: UIViewController {
    ...
    private var movieDetail: Search?
    private let context = appDelegate.persistentContainer.viewContext
    
    init(movieDetail: Search) {
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = movieDetail?.artworkUrl600 ?? movieDetail?.artworkUrl100
    ...
    private func setupUI() {
        searchName.text = "Name: \(movieDetail?.trackName ?? "")"
        searchPrice.text = "Price: \(movieDetail?.trackPrice ?? 0.0)" // ebookda formatted pricce kullan
        searchDescription.text = movieDetail?.longDescription ?? movieDetail?.shortDescription ?? movieDetail?.resultDescription ?? ""
        getImage(value: movieDetail?.artworkUrl600 ?? movieDetail?.artworkUrl100  ?? "")
    }
